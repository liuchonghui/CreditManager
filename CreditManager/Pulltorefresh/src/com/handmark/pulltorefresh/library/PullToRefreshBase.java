...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	/**ksz**/
	public Drawable topBackDrawable = null;
	/**ksz**/
	...
	/**ksz***/
	private boolean isHideFooterView;
	
	public void setHideFooterView(boolean isHideFooterView){
		this.isHideFooterView = isHideFooterView;
	}
...
//						else if (mMode.showFooterLoadingLayout() && diff <= -1f && isReadyForPullEnd()) {
						 ...
						 * ksz  根据变量屏蔽上拉刷新
						 */
						else if (!isHideFooterView&&mMode.showFooterLoadingLayout() && diff <= -1f && isReadyForPullEnd()) {
						/**
						 * ksz
						 */
							mLastMotionY = y;
	...
	protected LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs,int loadingType) {
		LoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(context, mode,
				getPullToRefreshScrollDirection(), attrs,loadingType);
...
//			}
	...
	protected final void setHeaderScroll(int value) {
		...
		/**
		 * ksz 解决自由滑动到底部和顶部时候跳动的BUG
		 */
		if(mState == State.OVERSCROLLING){
			if(value >= getFooterSize()/2)value = getFooterSize()/2;
			if(value <= -getHeaderSize()/2)value = -getHeaderSize()/2;
		}
		/**
		 * ksz
		 */
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mHeaderLayout = createLoadingLayout(context, Mode.PULL_FROM_START, a,LoadingLayout.LOADING_TYPE_HEADER);
		mFooterLayout = createLoadingLayout(context, Mode.PULL_FROM_END, a,LoadingLayout.LOADING_TYPE_FOOTER);
		...
		// recycle them...
		/**ksz**/
		if (a.hasValue(R.styleable.PullToRefresh_ptrReturnTopDrawable)) {
			topBackDrawable = a.getDrawable(R.styleable.PullToRefresh_ptrReturnTopDrawable);
		}
		/**ksz**/
	...
	public static enum AnimationStyle {
		...
		LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs
			...
			switch (this) {
				case ROTATE:
				default:
					return new RotateLoadingLayout(context, mode, scrollDirection, attrs,loadingType);
				case FLIP:
					return new FlipLoadingLayout(context, mode, scrollDirection, attrs,loadingType);
			}
		}
	}
