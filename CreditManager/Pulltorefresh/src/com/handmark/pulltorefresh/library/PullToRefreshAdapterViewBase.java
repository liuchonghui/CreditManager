...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	private void addIndicatorViews() {
		...
		if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			...
			mIndicatorIvTop = new IndicatorLayout(getContext(), Mode.PULL_FROM_START,topBackDrawable);
		...
		if (mode.showFooterLoadingLayout() && null == mIndicatorIvBottom) {
			// If the mode can pull down, and we don't have one set already
			mIndicatorIvBottom = new IndicatorLayout(getContext(), Mode.PULL_FROM_END,topBackDrawable);
			FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
					ViewGroup.LayoutParams.WRAP_CONTENT);
			params.rightMargin = getResources().getDimensionPixelSize(R.dimen.indicator_right_padding);
			params.gravity = Gravity.BOTTOM | Gravity.RIGHT;
			
			/**ksz  将滑动到底的向上图标改变成点击返回顶部的按钮**/
			params.setMargins(0, 0, 24, 50); 
			mIndicatorIvBottom.setOnClickListener(new OnClickListener() {
...
//			if (!isRefreshing() && isReadyForPullEnd()) {
	...
	/**ksz**/
	private boolean isShowReturnTop(){
		final Adapter adapter = mRefreshableView.getAdapter();

		if (null == adapter || adapter.isEmpty()) {
			return false;
		} else {
			final int childIndex = mRefreshableView.getFirstVisiblePosition();
			if (childIndex > 10) {
				return true;
			}
		}
